...
int main()
{
     ...
     */
    {
        ...
        int mas31_od[5] = {56, 484, -979, 56, 0};
        int mas31_dv_VSTR[2][3] = {{32, 0, -18}, {354, 234, 6}};

        int** mas31_dv = new int*[m];
        for(int i=0; i<m; i++){
            mas31_dv[i] = new int[n]; 
        }
        for(int i=0; i<m; i++){
            for(int j=0; j<n; j++){
                mas31_dv[i][j] = rand()%100;
                std::cout<< mas31_dv[i][j] << " ";
            }
            std::cout<<std::endl;
        }

        std::cout<<"В одномерном: " << min_odnomer(mas31_od, 5) << std::endl;
        std::cout<<"В двумерном: " << min_dvumer(mas31_dv, m, n) << std::endl;
        std::cout<<"В двумерном встроенном: " << min_dvumer_VSTR(&mas31_dv_VSTR[0][0], m, n) << std::endl;
        std::cout<< std::endl;
...

    {
        ...
        int date = 19, month = 3, year = 2020; 
        std::cout<<"Задание 4"<<std::endl;
        int day = DayOfYear(nDayTab[visokos(year)], date, month);
        std::cout<< day << std::endl;
        /** Проверка результата обратной функцией DayOfMonth */
        int date_1[2], date_2[2];
        //int* ptr1 = &date_1[0], ptr2 = &date_2[0];
        DayOfMonth(nDayTab[visokos(year)], day, date_1); //високосный
        DayOfMonth(nDayTab[visokos(year + 1)], day, date_2); //не високосный
        
        
        std::cout<< date_1[0] << "." << date_1[1] << std::endl; 
        std::cout<< date_2[0] << "." << date_2[1] << std::endl; 
        std::cout<< std::endl;
    }
    ...
    {
        int n = 3; // сформируйте значение n
     ...
     */
    {
        ...
        int len_decoded = decoded32_size(len_coded);
        char* decoded_data = new char[len_decoded];
        rez = decode32(dst, len_coded, decoded_data);
        std::cout<< "0 - декодировалось; 1 - нет. Результат: " << rez << std::endl;
